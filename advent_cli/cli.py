...
def main():
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=f"advent-cli {__version__}"
    )
    command_subparsers = parser.add_subparsers(
        dest="command",
        description="use advent {subcommand} --help for arguments",
        required=True,
    )
    ...
    parser_subparsers = parser_get.add_subparsers(
        dest="subcommand", help="use advent get puzzle date to get input", required=True
    )
    puzzle_parser = parser_subparsers.add_parser(
        "puzzle", help="use advent get puzzle date to get input"
    )
    solution_parser = parser_subparsers.add_parser(
        "solution", help="use advent get solution date to get input"
    )
    prompt_parser = parser_subparsers.add_parser(
        "prompt", help="re-read and overwrite the prompt from Advent of Code"
    )

    puzzle_parser.add_argument(
        "date",
        help='the year and day in YYYY/DD format (e.g. "2021/01") or year to '
        "get all puzzles from that year",
    )

    solution_parser.add_argument(
        "date",
        help='the year and day in YYYY/DD format (e.g. "2021/01") or "year" to'
        " get all solutions from that year",
    )

    prompt_parser.add_argument(
        "date", help='the year and day in YYYY/DD format (e.g. "2021/01")'
    )
    ...
    parser_submit = command_subparsers.add_parser(
        "submit", help="run solution and submit answers", formatter_class=CustomHelpFormatter
    )
    ...
    if args.command == "get":
        ...
        if args.subcommand == "puzzle":
            ...
            if day is None:
                print(colored('Date must include both year and day (e.g. "2021/01").', "red"))
            else:
                commands.refresh_prompt(year, day)
