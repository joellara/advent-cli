...
def get_solution(year, day):
    ...
    if r.status_code == 404:
        if "before it unlocks!" in r.text:
            ...
            print(
                colored(f"It will unlock on Dec {day} {year} at midnight EST (UTC-5).", "red")
            )
            print(
                colored(
                    f'Use "advent countdown {year}/{day}" to view a live countdown.', "grey"
                )
            )
            return
        else:
...
def refresh_prompt(year, day):
    ...
    if len(parts) == 0:
        print(colored("No prompt found on the page.", "red"))
        return
    ...
    print(f"Refreshed prompt to {year}/{day}/prompt.md")
...
def stats(year):
    ...
    for row in table_rows:
        stars_per_day[int(row[0]) - 1] = (
            2 if row[4:7] != ["-", "-", "-"] else 1 if row[1:4] != ["-", "-", "-"] else 0
        )
    ...
    if conf["private_leaderboards"]:
        ...
        print(
            colored(
                f"You are a member of {num_private_leaderboards} private leaderboard(s).",
                "grey",
            )
        )
...
def show_private_leaderboard(year, board_id):
    ...
    board_owner = (
        soup.find("div", class_="user").contents[0].strip()
    ...
    for row in rows:
        ...
        name_link = (
            row.select(".privboard-name a")[0].attrs["href"]
            if len(row.select(".privboard-name a"))
            else None
        )
...
def test(year, day, solution_file="solution", input_file=None):
    ...
    part1_answer, part2_answer = compute_answers(
        year, day, file_path=input_file, solution_file=solution_file
    )
...
def submit(year, day, solution_file="solution"):
    ...
    part1_answer, part2_answer = compute_answers(
        year,
        day,
        file_path=os.path.join(os.getcwd(), year, day, INPUT_FILE_NAME),
