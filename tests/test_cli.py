...
def test_cli_get(mock_argparse, mock_command_get):
    ...
    mock_argparse.return_value.parse_args.return_value.subcommand = "puzzle"
    cli.main()
...
def test_cli_test(mock_argparse, mock_command_test):
    ...
    mock_argparse.return_value.parse_args.return_value.input_file = "input.txt"
    cli.main()
    mock_command_test.assert_called_once_with(
        "2099", "99", solution_file="solution", input_file="input.txt"
    )
...
@patch("advent_cli.cli.commands.refresh_prompt")
@patch("argparse.ArgumentParser")
def test_cli_get_prompt(mock_argparse, mock_command_refresh_prompt):
    mock_argparse.return_value.parse_args.return_value.date = "2099/99"
    mock_argparse.return_value.parse_args.return_value.command = "get"
    mock_argparse.return_value.parse_args.return_value.subcommand = "prompt"
    cli.main()
    mock_command_refresh_prompt.assert_called_once_with("2099", "99")
